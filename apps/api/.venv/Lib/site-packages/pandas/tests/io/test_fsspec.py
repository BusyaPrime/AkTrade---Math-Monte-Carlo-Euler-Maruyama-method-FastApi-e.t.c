...
def test_feather_options(fsspectest):
    pytest.importorskip("pyarrow")
    df = DataFrame({"a": [0]})
    df.to_feather("testmem:
    assert fsspectest.test[0] == "feather_write"
    out = read_feather("testmem:
...
def test_pickle_options(fsspectest):
    df = DataFrame({"a": [0]})
    df.to_pickle("testmem:
    assert fsspectest.test[0] == "pickle_write"
    out = read_pickle("testmem:
...
def test_markdown_options(fsspectest):
    ...
    df = DataFrame({"a": [0]})
    df.to_markdown("testmem:
    assert fsspectest.test[0] == "md_write"
    assert fsspectest.cat("testmem:
